...
vector<vector<int>> get_demand(const char *path, vector<string> &customer_name)
{
    ...
    // 读文件（相对路径是相对与可执行文件）
    ifstream demand_csv(path);

    if (demand_csv.is_open())
    {
        getline(demand_csv, line);              // 读取第一行标题
        ...
        while (array)
        {
            customer_name.push_back(array);
            array = strtok(NULL, ",");
        }

        while (getline(demand_csv, line))       // 读取从第二行开始的数据
        {
            line = std::regex_replace(line, std::regex("\r"), ""); // 去掉字符串中的 \r
            strs = new char[line.length() + 1];
            strcpy(strs, line.c_str());         // 将 string 转化为 char*
            array = strtok(strs, ",");          // 将 char* 转化为数组
    ...
    {
        cout << "Open demand.csv failed." << endl;
        exit(-1);
    }
